...
function DispatchModel() {
  ...
  const { data } = dispatchAllDetailsByIdReducer
  ...
  const csvData = [
    ["Device Id", "Batch No", "Date Of Delivery", "Date Of Manuf.", "Hospital Name", "Purpose", "Serial No", "Concerned Person", "Phone Number", "Product Type", "Address"],
    [deviceid, batchNo, dod, dom, hospitalName, purpose, seiralNo, concernedPerson, PhoneNumber, productType, Address],
  ];
  return (
    <>
      <Navbar />
          ...
          <div className='mainContainer' style={{ display: 'flex', gap: '7rem', width: '200%' }}>
            <div className="container" style={{ width: "70%", display: "flex", flexDirection: "row", gap: "8rem", background: "#FFFFFF", boxShadow: "0px 0px 50px #00000029", borderRadius: "15px", padding: "2rem", marginLeft: "0px" }}>
              <div className="d-flex" style={{ gap: "1.5rem", flexDirection: "column", color: "#4B4B4B" }}>
                ...
                <h5 style={{ fontSize: "0.9rem" }}>Phone Number</h5>
              ...
              <div className="d-flex" style={{ gap: "1.5rem", flexDirection: "column", textAlign: "START", color: "#4B4B4B" }}>
                <h5 style={{ fontSize: "0.9rem" }}>{deviceid}</h5>
                <h5 style={{ fontSize: "0.9rem" }}>{batchNo}</h5>
                <h5 style={{ fontSize: "0.9rem" }}>{dod}</h5>
                <h5 style={{ fontSize: "0.9rem" }}>{dom}</h5>
                <h5 style={{ fontSize: "0.9rem" }}>{hospitalName}</h5>
                <h5 style={{ fontSize: "0.9rem" }}>{purpose}</h5>
                <h5 style={{ fontSize: "0.9rem" }}>{seiralNo}</h5>
                <h5 style={{ fontSize: "0.9rem" }}>{concernedPerson}</h5>
                <h5 style={{ fontSize: "0.9rem" }}>{PhoneNumber}</h5>
                <h5 style={{ fontSize: "0.9rem" }}>{productType}</h5>
                <h5 style={{ fontSize: "0.9rem" }}>{Address}</h5>
              </div>
            </div>
