    ...
    <div>
      <Row className="rowSection">
        ...
        >
          ...
          <div>
          <h3 className='heading' style={{paddingTop:"6rem" , fontsize: "1.9rem", color:"#0099a4", marginLeft:"50rem"}}>Agva Pro ventilator</h3>
                                            ...
                                            return (
                                              <React.Fragment key={_id}>
                                                ...
                                                <section className='d-flex' style={{ gap: '5px' }}>
                                                  ...
                                                  /></>}
                                                  <Button title='Next' style={{ marginLeft: "13px" }} onClick={() => {
                                                    navigate(`/deviceEvents?code=${code}&projectName=${projectName}&DeviceId=${item.did}`)
                                                    { item1 }
                                                    // console.log({ ...item1 })
                                                    { localStorage.setItem('item1', JSON.stringify(item1)) }
                                                    { localStorage.setItem('AliasName', JSON.stringify(item1.AliasName)) }
                                                    { localStorage.setItem('Hospital_Name', JSON.stringify(item1.Hospital_Name)) }
                                                    { localStorage.setItem('Doctor_Name', JSON.stringify(item1.Doctor_Name)) }
                                                    { localStorage.setItem('Ward_No', JSON.stringify(item1.Ward_No)) }
                                                    { localStorage.setItem('IMEI_NO', JSON.stringify(item1.IMEI_NO)) }
                                                    { localStorage.setItem('Ventilator_Operator', JSON.stringify(item1.Ventilator_Operator)) }
                                                  }}><FontAwesomeIcon
                                                      style={{ cursor: 'pointer' }}
                                                      icon={faArrowRight}
                                                    /></Button>
