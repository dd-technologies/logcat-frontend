        ...
        >
          ...
          <div className="container" style={{display:"flex",flexDirection:"row",gap:"8rem",background: "#FFFFFF 0% 0% no-repeat padding-box",boxShadow: "0px 0px 50px #00000029", borderRadius:"15px",padding: "2rem", marginLeft:"0px"}}>
            ...
            {aboutFilter && aboutFilter.map((item,_id)=>{
              return(
              <>
                <h5 style={{fontSize:"1rem"}}>{!item.product?item.product=="--":item.product}</h5>
                <h5 style={{fontSize:"1rem"}}>{!item.model?item.model=="--":item.model}</h5> 
                <h5 style={{fontSize:"1rem"}}>{!item.delivery_date?item.delivery_date=="--":item.delivery_date}</h5>
                <h5 style={{fontSize:"1rem"}}>{!item.batch_no?item.batch_no=="--":item.batch_no}</h5>
                <h5 style={{fontSize:"1rem"}}>{!item.date_of_warranty?item.date_of_warranty=="--":item.date_of_warranty}</h5>
                <h5 style={{fontSize:"1rem"}}>{!item.last_service?item.last_service=="--":item.last_service}</h5>
                </>
              )
              })}
