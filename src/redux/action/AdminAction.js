  ...
  (email, password, isRemeberMe) => async (dispatch) => {
    try {
      ...
      const { data } = await axios.post(
        `https://logger-server.herokuapp.com/api/logger/login`,
        {
          email,
          password,
        },
...
export const adminLogout = (history) => async (dispatch) => {
  try {
    ...
    await axios.get(
      `https://logger-server.herokuapp.com/api/logger/logout`,
      config
    );
...
export const adminRegister =
  (email, password, name, history) => async (dispatch) => {
    try {
      ...
      const { data } = await axios.post(
        "https://logger-server.herokuapp.com/api/logger/register",
        {
          name,
          email,
          password,
...
export const forgetPassword = (email) => async (dispatch) => {
  try {
    ...
    // https://insulink-backend.herokuapp.com
    const { data } = await axios.post(
      "https://logger-server.herokuapp.com/api/logger/forget",
      {
        email,
      }
    );
...
export const resetForgetPassword =
  ...
  async (dispatch) => {
    try {
      ...
      // https://insulink-backend.herokuapp.com
      const { data } = await axios.post(
        "https://logger-server.herokuapp.com/api/logger/resetPassword",
        {
          otp,
          password,
          email,
...
export const updateProfile = (email, name, avatar) => async (dispatch) => {
  try {
    ...
    const { data } = await axios.put(
      `http://localhost:5000/api/logger/update`,
      formData,
      config
    );
    // console.log(data);
