...
export const deviceAction = (code) => async (dispatch) => {
  try {
      dispatch({
        type:DEVICE_REQUEST,
      });
...
export const getCalibrationById = () => async (dispatch) => {
  try {
      dispatch({
      ...
      response = await axios.get(
        `${process.env.REACT_APP_BASE_URL}/api/logger/logs/calibration/${DeviceId12}`,
        config
      );
      dispatch({
